...
func TestGPUServerSetupStages(t *testing.T) {
	...
	// Without network commands and model, should have: install-deps, build-llama-cpp, build-tanrenai-gpu, start-gpu-server
	if len(stages) != 4 {
		t.Fatalf("got %d stages, want 4", len(stages))
	}

	names := []string{"install-deps", "build-llama-cpp", "build-tanrenai-gpu", "start-gpu-server"}
...
func TestGPUServerSetupStagesWithNetwork(t *testing.T) {
	...
	// Should have: install-deps, build-llama-cpp, build-tanrenai-gpu, pull-model, setup-network, start-gpu-server
	if len(stages) != 6 {
