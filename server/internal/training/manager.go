...
func NewManager(client *SidecarClient, memStore memory.Store) *Manager {
	return &Manager{
...
func NewManagerWithStore(store *RunStore, client *SidecarClient, memStore memory.Store, modelsDir string) *Manager {
	return &Manager{
		runStore:  store,
		client:    client,
		memStore:  memStore,
...
// Prepare exports a dataset from memory and creates a pending training run.
func (m *Manager) Prepare(ctx context.Context, baseModel string, cfg RunConfig) (*TrainingRun, error) {
	runID := fmt.Sprintf("%s-%d", "ft", time.Now().Unix())

	datasetPath, sampleCount, err := ExportDataset(ctx, m.memStore, runID, cfg.MaxSamples)
	if err != nil {
...
func (m *Manager) Merge(ctx context.Context, runID string, outputName string) (string, error) {
	...
	if run.Status != StatusMerging && run.Status != StatusDone {
		// Also allow merging a run whose training is reported complete by the sidecar
		if run.Status == StatusTraining {
			status, err := m.client.Status(ctx, runID)
